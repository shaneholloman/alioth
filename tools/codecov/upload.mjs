...
async function saveToDisk(outDir, payload, prNumber, commentBody) {
  console.log(`\n[3/4] Writing payloads to disk...`);
  console.log(`      Output Directory: ${outDir}`);

  await fs.mkdir(outDir, { recursive: true });
  await fs.writeFile(
    path.join(outDir, "codecov_payload.json"),
    JSON.stringify(payload),
  );

  if (prNumber) {
    const commentPayload = { body: commentBody };
    await fs.writeFile(
      path.join(outDir, "github_comment.json"),
      JSON.stringify(commentPayload),
    );
  }

  console.log(`\n[4/4] Data generated successfully in ${outDir}!`);
}

async function postData(apiUrl, apiToken, payload, prNumber, commentBody) {
  ...
  try {
    ...
    if (prNumber) {
      await postCommentToGithub(prNumber, commentBody);
    }
  } catch (err) {
...
async function postCommentToGithub(prNumber, body) {
...
async function main() {
  ...
  if (!lcovPath) lcovPath = "lcov.info";

  const baseUrl = process.env.CODECOV_URL || "http://localhost:8787";
  ...
  const commentBody = generateMarkdownComment(
    prNumber,
    commit_sha,
    coverage_pct,
    delta_coverage_pct,
  ...
  if (outDir) {
    await saveToDisk(outDir, payload, prNumber, commentBody);
  } else {
    await postData(apiUrl, apiToken, payload, prNumber, commentBody);
  }
}
